...
Deno.serve(async (req) => {
  try {
    ...
    const { jobImportId, parsedData } = body;

    if (!jobImportId || !parsedData) {
      return Response.json({ error: 'Missing required fields' }, { status: 400 });
    }

    // Fetch JobImport and validate ownership
    const jobImport = await base44.asServiceRole.entities.JobImport.get(jobImportId);
    if (!jobImport) {
      return Response.json({ error: 'Import not found' }, { status: 404 });
    }

    if (jobImport.customer_id !== user.customer_id) {
      return Response.json({ error: 'Unauthorized: Cannot replace another customer\'s import' }, { status: 403 });
    }

    // Delete all existing jobs from this import
